...
<body>
    <div class="p-40 bg-amber-50 min-h-screen">
        <h1 class="text-2xl font-bold">Chỉnh sửa Group: {{ $group->name }}</h1>
        <div class="mt-4">
        ...
        <div class="mt-4 bg-white p-6 rounded-md">
            <form action="{{ route('groups.update', $group) }}" method="post">
                            ...
                            <div class="p-2 hover:bg-gray-100">
                                <label class="flex items-center space-x-2">
                                    <input type="checkbox" name="user_ids[]" value="{{ $user->id }}"
                                        {{ in_array($user->id, $group->users->pluck('id')->toArray()) ? 'checked' : '' }} class="rounded">
                ...
                <button type="submit" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md">Cập nhật Group</button>
            </form>
        </div>
    </div>
</body>
